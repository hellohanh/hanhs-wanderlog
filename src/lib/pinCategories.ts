...
// `color`, when set, overrides the category's badge color for that
// specific variant (see pinBadgeColor below) — e.g. Attraction's
// General/star stays the category's navy, but Church/Museum/etc. each
// get their own distinct color so the map reads at a glance.
export const ICON_VARIANTS: Partial<Record<PinCategory, IconVariant[]>> = {
  attraction: [
    { key: 'general', label: 'General', svg: ICON_SVGS.attraction, color: '#1B2A4A' },
    {
      key: 'church',
      label: 'Church',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="3" x2="12" y2="21"/><line x1="7" y1="9" x2="17" y2="9"/></svg>`,
      color: '#8B1E3F'
    },
    {
      key: 'museum',
      label: 'Museum',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 21h16"/><path d="M5 21V10M9 21V10M15 21V10M19 21V10"/><path d="M3 10l9-6 9 6"/></svg>`,
      color: '#B8860B'
    },
    {
      key: 'landmark',
      label: 'Landmark',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3 6h-6l3-6z"/><path d="M9 8h6l2 6H7l2-6z"/><path d="M6 21V14h12v7"/></svg>`,
      color: '#B5541F'
    },
    {
      key: 'temple',
      label: 'Temple/pagoda',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l4 5H8l4-5z"/><path d="M9 7l5 6H6l3-6z"/><path d="M4 21l4-8h8l4 8z"/></svg>`,
      color: '#C0392B'
    },
    {
      key: 'viewpoint',
      label: 'Viewpoint',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="17" cy="6" r="2" fill="white" stroke="none"/><path d="M3 20l6-9 4 5 3-4 5 8"/></svg>`,
      color: '#2E86AB'
    },
    {
      key: 'park',
      label: 'Park/garden',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="5"/><line x1="12" y1="14" x2="12" y2="21"/></svg>`,
      color: '#4C8C4A'
    },
    {
      key: 'bridge',
      label: 'Bridge',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 16c2-4 5-6 9-6s7 2 9 6"/><line x1="5" y1="16" x2="5" y2="21"/><line x1="19" y1="16" x2="19" y2="21"/><line x1="3" y1="21" x2="21" y2="21"/></svg>`,
      color: '#5C6B73'
    },
    {
      key: 'music_venue',
      label: 'Music venue',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l11-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="17" cy="16" r="3"/></svg>`,
      color: '#8E44AD'
    }
  ],
  dining: [
    ...
    },
    {
      key: 'seafood',
      label: 'Seafood',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12c4-5 12-5 16 0-4 5-12 5-16 0z"/><circle cx="16" cy="11" r="0.8" fill="white" stroke="none"/><path d="M19 12l3-3M19 12l3 3"/></svg>`
    },
    {
      key: 'fast_food',
      label: 'Fast food',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 10h16"/><path d="M4 14h16"/><path d="M5 10a7 4 0 0 1 14 0"/><path d="M5 14a7 3 0 0 0 14 0"/></svg>`
    },
    {
      key: 'fine_dining',
      label: 'Fine dining',
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3h8l-1 6a3 3 0 0 1-6 0z"/><line x1="12" y1="12" x2="12" y2="19"/><line x1="8" y1="21" x2="16" y2="21"/></svg>`
    },
    {
      key: 'bar_pub',
      label: 'Bar/pub',
...
// Badge/circle color for a pin — a variant's own `color` when it has
// one (see ICON_VARIANTS), otherwise the category's default color.
// This is what actually makes icon variants visually distinct on the
// map, not just internally different glyphs.
export function pinBadgeColor(category: PinCategory, iconKey: string | null): string {
  if (iconKey) {
    const variant = ICON_VARIANTS[category]?.find(v => v.key === iconKey)
...
// Label for a pin's specific icon variant (e.g. "Church"), used to
// group/sort the pinned list by icon type within a category, not just
// by name. Categories without variants (ICON_VARIANTS[category] is
// undefined) return '' — with only one possible icon, this sort key is
// a no-op for them and falls straight through to alphabetizing by name.
export function pinIconLabel(category: PinCategory, iconKey: string | null): string {
  const variants = ICON_VARIANTS[category]
  if (!variants) return ''
  const match = variants.find(v => v.key === (iconKey ?? 'general'))
...
// alphabetical by label) with pins alphabetized within each group —
// primarily by icon-variant label (e.g. all "Church" pins together,
// then "Museum", etc.), and by name as the tiebreaker within a given
// icon type. Categories with no pins in the list are omitted. Shared
// by the Map tab's pinned list and the Itinerary tab's unscheduled
// pool so both group/sort identically.
export function groupPinsByCategory(pins: Pin[]): PinCategoryGroup[] {
  return CATEGORIES.map(cat => ({
    ...
    pins: pins
      ...
      .sort((a, b) => {
        const iconCompare = pinIconLabel(a.category, a.icon).localeCompare(
          pinIconLabel(b.category, b.icon),
          undefined,
          { sensitivity: 'base' }
        )
